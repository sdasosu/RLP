...
def _infer_conv_type(m: nn.Module) -> Optional[str]:
    if isinstance(m, (nn.Conv1d, nn.Conv2d, nn.Conv3d)):
        ...
        if g == m.in_channels and m.out_channels % m.in_channels == 0 and g > 1:
            return "depthwise"
...
class DependencyGraph:
    ...
    def _make_module_hook(self, name: str, module: nn.Module):
        def hook(mod, inputs, output):
            node = self._new_node(
                ...
                conv_type=_infer_conv_type(module),
                is_module=True,
            )
    ...
    def _patch(self):
        ...
        self._orig = {
            ...
            "radd": torch.Tensor.__radd__,
            "iadd": torch.Tensor.__iadd__,
            "sub": torch.Tensor.__sub__,
            "rsub": torch.Tensor.__rsub__,
            "isub": torch.Tensor.__isub__,
            "mul": torch.Tensor.__mul__,
            "rmul": torch.Tensor.__rmul__,
            "imul": torch.Tensor.__imul__,
            "truediv": torch.Tensor.__truediv__,
            "rtruediv": torch.Tensor.__rtruediv__,
            "itruediv": torch.Tensor.__itruediv__,
            "matmul": torch.Tensor.__matmul__,
            "rmatmul": torch.Tensor.__rmatmul__,
            ...
            "relu": F.relu,
            "gelu": getattr(F, "gelu", None),
            "silu": getattr(F, "silu", None),
            "sigmoid": F.sigmoid,
            "softmax": F.softmax,
        ...
        def T(op, out, ins_tensors):
            node = dg._new_node(
                name=f"{op}_{dg._op_idx}", typ=op, conv_type=None, is_module=False
            )
            dg._op_idx += 1
            parents = set()
        ...
        # binary
        def t_add(a, b):
            return T("add", self._orig["add"](a, b), [a, b])

        def t_radd(a, b):
            return T("add", self._orig["radd"](a, b), [a, b])

        def t_iadd(a, b):
            return T("add_", self._orig["iadd"](a, b), [a, b])

        def t_sub(a, b):
            return T("sub", self._orig["sub"](a, b), [a, b])

        def t_rsub(a, b):
            return T("sub", self._orig["rsub"](a, b), [a, b])

        def t_isub(a, b):
            return T("sub_", self._orig["isub"](a, b), [a, b])

        def t_mul(a, b):
            return T("mul", self._orig["mul"](a, b), [a, b])

        def t_rmul(a, b):
            return T("mul", self._orig["rmul"](a, b), [a, b])

        def t_imul(a, b):
            return T("mul_", self._orig["imul"](a, b), [a, b])

        def t_div(a, b):
            return T("div", self._orig["truediv"](a, b), [a, b])

        def t_rdiv(a, b):
            return T("div", self._orig["rtruediv"](a, b), [a, b])

        def t_idiv(a, b):
            return T("div_", self._orig["itruediv"](a, b), [a, b])

        def t_matmul(a, b):
            return T("matmul", self._orig["matmul"](a, b), [a, b])

        def t_rmatmul(a, b):
            return T("matmul", self._orig["rmatmul"](a, b), [a, b])

        # shape/layout
        def t_view(x, *s):
            return T("view", self._orig["view"](x, *s), [x])

        def t_reshape(x, *s):
            return T("reshape", self._orig["reshape"](x, *s), [x])

        def t_permute(x, *d):
            return T("permute", self._orig["permute"](x, *d), [x])

        def t_flatten(x, a=0, b=-1):
            return T("flatten", self._orig["flatten"](x, a, b), [x])

        def t_transpose(x, i, j):
            return T("transpose", self._orig["transpose"](x, i, j), [x])

        # joins
        def t_cat(ts, dim=0):
            return T("cat", self._orig["cat"](ts, dim), list(ts))

        def t_stack(ts, dim=0):
            return T("stack", self._orig["stack"](ts, dim), list(ts))
        ...
        def t_gelu(x, approximate="none"):
            y = (
                self._orig["gelu"](x, approximate=approximate)
                if self._orig["gelu"]
                else torch.nn.GELU()(x)
            )
            return T("gelu", y, [x])
        ...
        def t_sigmoid(x):
            return T("sigmoid", self._orig["sigmoid"](x), [x])

        def t_softmax(x, dim=None):
            return T("softmax", self._orig["softmax"](x, dim=dim), [x])

        # others
        def t_interpolate(x, *a, **k):
            return T("interpolate", self._orig["interpolate"](x, *a, **k), [x])

        def t_linear(x, w, b=None):
            return T("linear", self._orig["linear"](x, w, b), [x])

        def t_avgpool(x, *a, **k):
            return T("avg_pool2d", self._orig["avg_pool2d"](x, *a, **k), [x])

        def t_maxpool(x, *a, **k):
            return T("max_pool2d", self._orig["max_pool2d"](x, *a, **k), [x])
        ...
        torch.stack = t_stack
        F.relu = t_relu
        if self._orig["gelu"] is not None:
            F.gelu = t_gelu
        if self._orig["silu"] is not None:
            F.silu = t_silu
        F.sigmoid = t_sigmoid
        F.softmax = t_softmax
    ...
    def _unpatch(self):
        ...
        for k, v in self._orig.items():
            if k in ("gelu", "silu") and v is None:
                continue
        ...
        if self._orig["gelu"] is not None:
            F.gelu = self._orig["gelu"]
        if self._orig["silu"] is not None:
            F.silu = self._orig["silu"]
    ...
    def print_graph(self, limit: int = 60):
        ...
        if len(self.nodes) > limit:
            print(f"... and {len(self.nodes) - limit} more")
        print("=" * 80)
