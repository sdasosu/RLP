...
#============ core ==============
def run_fake_rl(model_, train_loader, test_loader, cutmix_or_mixup,
                checkpoint_path=None, sparsity=0.5, seed=42, device=None):
    if device is None:
...
if __name__ == "__main__":
    ...
    from helper.model_zoo import resnet18
    ...
    out = run_fake_rl(
        model_, train_loader, test_loader, cutmix_or_mixup,
        checkpoint_path="./assets/checkpoint/baselines/resnet18_cifar10_pruned.pt",
        sparsity=0.5, seed=42, device=device
    )
