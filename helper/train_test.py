...
def _resolve_ckpt_path(state_dict_path: str,
                       model: nn.Module,
                       dataname: str) -> str:
    """
    ...
    """
    os.makedirs(state_dict_path if not state_dict_path.endswith((".pt", ".pth")) else os.path.dirname(state_dict_path) or ".", exist_ok=True)
...
# Public callables
# =========================
def _test(model: nn.Module,
          test_loader,
          device: str = "cuda",
          state_dict_path: Optional[str] = None) -> float:
    """
...
def _training(model: nn.Module,
              ...
              device: str = "cuda",
              state_dict_path: str = "./",
              dataname: Optional[str] = None) -> Tuple[nn.Module, float]:
    """
    ...
    final_test_acc = _test(model, test_loader, device=device, state_dict_path=best_ckpt_path)
