...
def _node_kind(n: fx.Node, name_to_mod: Dict[str, nn.Module]) -> str:
    """
    'channel_affect' : conv/linear/convT
    'pass'           : relu/bn/pool/flatten etc.
    'merge_add'      : residual add
    'merge_cat'      : concatenation (e.g., Inception)
    'other'
    """
    if n.op == "call_module":
        m = name_to_mod[n.target]
        if isinstance(m, _CHANNEL_AFFECTING): return "channel_affect"
...
def _weight_key_for(n: fx.Node, name_to_mod: Dict[str, nn.Module]) -> str:
    """Return 'module_path.weight' for layers that own weights; '-' otherwise."""
    if n.op == "call_module":
        m = name_to_mod[n.target]
...
def _build_edges(gm: fx.GraphModule) -> List[Dict[str, Any]]:
    ...
    # index users for traversal
    users: Dict[fx.Node, List[fx.Node]] = {n: [] for n in nodes}
    for n in nodes:
        for u in n.users:
            users[n].append(u)
...
if __name__ == "__main__":
    ...
    for name, ctor in [
        ...
        pretty_print(focus[:] if focus else rows[:])